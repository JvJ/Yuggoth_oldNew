(ns yuggoth.blang
  ...
  (:require [yuggoth.physics-util :refer :all]
            [clojure.walk :as wk])
...
(defn rect
 ...
 ratio of its width to its height.  Default alignment is center"
  [&{:keys [align width height] :or {align :center width 1 height 1}}]
  {:pre [(valid-align-flag? align)]}
  #(let [verts (->> [(v2 0,0) (v2 width,0) (v2 width,height) (v2 0,height)]
                   (v-shift (case align
                              :center (v2 (- (/ width 2.0 )),(* -0.5 height) )
                              :left (v2 0,(* -0.5 height) )
                              :right (v2 (* -2 width),(* -0.5 height) )
                              :top (v2 (- (/ width 2.0)),(- height))
                              :bottom (v2 (- (/ width 2.0)),0)))
                   (map transform))
        ret (PolygonShape.)
        _ (. ret (set (v2-to-floats verts)))]
    ret))
...
(defn circ
 ...
 centered."
  ...
  #(let [vert (case align :center (v2 0,0) :left (v2 1,0)
         ...
         vert (transform vert)
         ret (CircleShape.)
         _ (. ret (setRadius (. (.sub (transform (v2 1,0)) vert) (len))))
